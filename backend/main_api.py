...
async def execute_library_cheapest_book_by(
    ...
    try:
        ...
        async def wrapper(_library_object):
            cheapest: Book = None
            price = 1000000000.0
            book_8757674694356 = (await get_books_of_library(_library_object.id, database))['books']
            for i in range(0, len(book_8757674694356)):
                book = book_8757674694356[i]
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        stock=book_data.stock,        time=book_data.time,        release=book_data.release,        price=book_data.price,        pages=book_data.pages,        title=book_data.title        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                stock=item_data.stock,                time=item_data.time,                release=item_data.release,                price=item_data.price,                pages=item_data.pages,                title=item_data.title            )
            database.add(db_book)
            database.flush()  # Get ID without committing
